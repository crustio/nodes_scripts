...
function usage()
{
    ...
    echo "     -s|--spower <number>: Required, indicate add spower whose unit must be g/G/t/T, cannot be set with -n"
...
owned_cids_file=$basedir/owned_cids
add_files_script=$basedir/add_files.sh
prev_meta_file=$basedir/prev_meta
...
if $RESET; then
    rm $prev_meta_file $owned_cids_file &>/dev/null
fi
if $GETNEW; then
    break_cond=0
fi
...
account="cTGUcv9jhTBmaw6FZiih7TNTQnmgtSQzFFurYgDXHQBeFw7MT"
if [ x"$account" = x"" ]; then
...
if [ x"$owner_id" = x"" ] || [ ${#owner_id} -lt ${#account} ]; then
    echo "ERROR: cannot get owner account according to current account:'$account', please try again"
    exit 1
fi
...
for id in ${member_ids[@]}; do
    ...
    if ! cat $TMPFILE | jq . &>/dev/null; then
        echo " No new files can be found from block:${id_2_lblk[$id]}"
        if [ $myacc -gt 2 ]; then break; fi
        continue
    fi
    declare -A acc_map
    ...
    for el in $(cat $TMPFILE | jq -r ".data.accountById.workReports|.[]|.addedFiles|.[]|.[0,2]"); do
        if ((j % 2 == 0 )); then
            ...
            lest_blk_num=$(max $lest_blk_num $el)
        fi
        ((j++))
    done
    ((file_num=file_num+${#acc_map[@]}))
    echo " Done: file number(current:${#acc_map[@]}, total:$file_num), latest block:$lest_blk_num"
    unset acc_map
    if [ $lest_blk_num -gt 0 ]; then
        id_2_lblk[$id]=$lest_blk_num
    fi
    fetched_latest_blk=$(max $fetched_latest_blk $lest_blk_num)
    if [ $myacc -gt 2 ]; then break; fi
done
...
echo "INFO: latest block number:$fetch_start_blk_n"
...
while ! $enough ; do
    ...
    if [ $file_num -gt 0 ]; then
        ...
        echo "INFO: no more files can be found"
        break
    fi
done
if [ $break_cond -eq 0 ]; then
...
EOF
    for id in ${!id_2_cids[@]}; do
        echo "id_2_cids[$id]=\"${id_2_cids[$id]}\"" >> $add_files_script
        #echo "$id: ${id_2_size[$id]}"
    done
cat << EOF >> $add_files_script
...
for id in ${!id_2_lblk[@]}; do
    echo "id_2_lblk[$id]=${id_2_lblk[$id]}" >> $prev_meta_file
done
echo "fetch_start_blk_o=$fetch_start_blk_o" >> $prev_meta_file
echo "fetch_start_blk_n=$fetch_start_blk_n" >> $prev_meta_file
